...
Use Wave 1 from that plan as the next implementation PR. Do not skip straight to
launching Codex, Claude Code, OpenCode, OMX, or cloud workers; the first wave is
non-executing `team-launch-preflight`. Lane 1C has a docs/contract placeholder at
`docs/team-launch-preflight/`; its JSON fixtures must be generated by
`python3 -m depone team-launch-preflight` once Lane 1A/1B make that command
available, not copied from older Team Ledger artifacts.
