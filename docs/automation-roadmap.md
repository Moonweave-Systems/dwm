...
| Agent Fabric paired run | Codex direct-vs-governed runner receipts plus Depone observer captures for one real task | planned V126, first real paired run implemented |
| Verify claim honesty | fail-closed claim evaluation where unevaluated required claims are inconclusive | planned V127, verify self-test coverage implemented |
| Evidence substrate | in-toto Statement, unsigned DSSE envelope, and OTel GenAI-shaped span bundle from observed capture evidence | planned V128, first stdlib-only bundle implemented |
...
Status: implemented through V128 first evidence substrate; the V103 live two-arm
...
1. Harden V128 ingest: accept external in-toto/DSSE statements and OTel span
   bundles as untrusted inputs, verify subject digests against present artifacts,
   and return `inconclusive` on mismatch.
2. Dogfood the installed `depone` command on another real local maintenance task
   and emit runner receipt, observer capture, evidence substrate bundle, and a
   decision note.
3. Improve local install and quickstart flow without publishing to PyPI or
   renaming packages until an install/migration gate proves compatibility.
4. Expand read-only or pre-isolated live execution only where V84/V85 and queue
   preflight evidence permit it.
5. Keep V124 Agent OS / role-profile expansion parked unless it directly helps
   capture, ingest, verify, or trust real evidence.
