...
def make_narrative(
    ...
    *,
    ...
    if roadmap_latest != LATEST_ROADMAP_VERSION:
        add_blocker(blockers, "ERR_WORKFLOW_NARRATIVE_ROADMAP_STALE", "roadmap latest version is stale", surface="roadmap", actual=roadmap_latest, expected=LATEST_ROADMAP_VERSION)
    ...
    if activation_inputs.get("roadmap_latest_version") != LATEST_ROADMAP_VERSION:
        add_blocker(blockers, "ERR_WORKFLOW_NARRATIVE_ACTIVATION_ROADMAP_STALE", "activation consumed a stale roadmap version", surface="activation", actual=activation_inputs.get("roadmap_latest_version"), expected=LATEST_ROADMAP_VERSION)
    ...
    stages = [
        stage(
            "chart",
            "Chart",
            "clear" if roadmap.get("decision") == "roadmap_reconciled" and roadmap_latest == LATEST_ROADMAP_VERSION and not roadmap.get("blocked_by") else "blocked",
...
def ready_records() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    roadmap = {"decision": "roadmap_reconciled", "blocked_by": [], "policy": {"latest_version": LATEST_ROADMAP_VERSION}}
    command_safety = {"decision": "keep", "failed": 0, "required_fixture_count": 4, "required_passed": 4}
    activation = {
        "decision": "ready_for_next_workflow_design",
        "blocked_by": [],
        "inputs": {"roadmap_latest_version": LATEST_ROADMAP_VERSION},
